...
const IGNORED_KEYS = new Set(['createdAt', 'updatedAt', 'publishedAt', 'status_code', 'locale']);
...
export function diffCartById(initial: OrderItem[], current: OrderItem[]) {
  const byIdInitial = new Map<number, OrderItem>(initial.map((it) => [it.id, it]));
  const byIdCurrent = new Map<number, OrderItem>(current.map((it) => [it.id, it]));
  ...
  for (const [id, oldItem] of byIdInitial) {
    ...
    if (patch.selected_options !== undefined || patch.quantity !== undefined) {
      update.push(patch);
    }
  }
