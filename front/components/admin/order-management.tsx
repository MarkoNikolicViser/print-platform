                ...
                <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                  <Box flex={1}>
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid size={{ xs: 12, md: 4 }}>
                        <Typography variant="caption" color="text.secondary">Detalji:</Typography>
                        ...
                        <Box display="flex" gap={1} mt={1}>
                          ...
                          {order.isDoubleSided && <Chip label="Obostrano" size="small" variant="outlined" />}
                          {order.binding !== "none" && <Chip label={order.binding} size="small" variant="outlined" />}
                        </Box>
                      </Grid>
                      <Grid size={{ xs: 12, md: 4 }}>
                        <Typography variant="caption" color="text.secondary">Vreme:</Typography>
                        <Typography variant="body2">Naručeno: {order.createdAt}</Typography>
                        <Typography variant="body2">Završetak: {order.estimatedCompletion}</Typography>
                      </Grid>
                    </Grid>
                  </Box>
                  ...
                  <Box textAlign="right" ml={2}>
                    ...
                    <Box display="flex" flexDirection="column" gap={1}>
                      {order.status === "pending" && (
                        <Button size="small" variant="contained" onClick={() => updateOrderStatus(order.id, "processing")}>
                          <Clock size={16} style={{ marginRight: 4 }} />
                          Počni
                        </Button>
                      )}
                      {order.status === "processing" && (
                        <Button size="small" variant="contained" onClick={() => updateOrderStatus(order.id, "ready")}>
                          <CheckCircle size={16} style={{ marginRight: 4 }} />
                          Završi
                        </Button>
                      )}
                      {order.status === "ready" && (
                        <Button size="small" variant="contained" onClick={() => updateOrderStatus(order.id, "completed")}>
                          Preuzeto
                        </Button>
                      )}
                      <Button size="small" variant="outlined" onClick={() => sendNotification(order)}>
      ...
      {selectedOrder && (
        <Dialog open={true} onClose={() => setSelectedOrder(null)} fullWidth maxWidth="md">
          <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <Button variant="outlined" onClick={() => setSelectedOrder(null)}>Zatvori</Button>
          </DialogTitle>
          <DialogContent dividers>
            <Grid container spacing={4}>
              <Grid size={{ xs: 12, md: 6 }}>
                ...
                <Typography><strong>Ime:</strong> {selectedOrder.customer}</Typography>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                ...
                <Typography><strong>Fajl:</strong> {selectedOrder.fileName}</Typography>
                <Typography><strong>Stranica:</strong> {selectedOrder.pages}</Typography>
                <Typography><strong>Količina:</strong> {selectedOrder.quantity} primeraka</Typography>
                <Typography><strong>Kopije:</strong> {selectedOrder.copies} po primeru</Typography>
                <Typography><strong>Boja:</strong> {selectedOrder.isColor ? "Da" : "Ne"}</Typography>
                <Typography><strong>Obostrano:</strong> {selectedOrder.isDoubleSided ? "Da" : "Ne"}</Typography>
                <Typography><strong>Povezivanje:</strong> {selectedOrder.binding}</Typography>
                <Typography><strong>Ukupno:</strong> {selectedOrder.amount} RSD</Typography>
              </Grid>
            </Grid>
          </DialogContent>
        </Dialog>
