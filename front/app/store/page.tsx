...
import { BarChart3, ShoppingCart, DollarSign, Settings } from 'lucide-react';

const menuItems = [
  // { id: 'dashboard', label: 'Pregled', icon: <BarChart3 size={20} /> },
  { id: 'orders', label: 'Narudžbine', icon: <ShoppingCart size={20} />, disabled: false, needsAttention: false },
  { id: 'pricing', label: 'Cene', icon: <DollarSign size={20} />, disabled: false, needsAttention: false },
  { id: 'settings', label: 'Podešavanja', icon: <Settings size={20} />, disabled: false, needsAttention: false },
];
...
export default function StorePage() {
  ...
  useEffect(() => {
    ...
    const hasAnyActivePricing = templates.some(
      (t) => t.pricing && t.pricing.is_active
    );
