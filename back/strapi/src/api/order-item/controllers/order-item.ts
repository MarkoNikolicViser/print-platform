...
module.exports = {
  ...
  async sync(ctx) {
    ...
    if (!order.print_shop_id)
      return ctx.badRequest("Order does not have a print_shop assigned");
    ...
    /** 4️⃣ DELETE ITEMS (robust two-step + logging) */
    if (deletedIds && deletedIds.length) {
      // Normalize types (SQLite can be picky about numeric vs string)
      ...
      try {
        // Step 1: fetch only those item IDs that actually belong to this order
        const rows = await strapi.db
          .query("api::order-item.order-item")
          .findMany({
            select: ["id"],
            where: {
              id: { $in: normalizedDeletedIds },
              order: { id: normalizedOrderId }, // explicit relation filter
            },
          });
        ...
        if (!ownedIds.length) {
          // Nothing to delete for this order; bail out silently
        } else if (ownedIds.length === 1) {
          // Some databases/ORMs are more predictable with single deletes
          await strapi.db.query("api::order-item.order-item").delete({
            where: { id: ownedIds[0] },
          });
        } else {
          // Step 2: delete by primary key only (no JOIN), avoids ambiguous SQL
          await strapi.db.query("api::order-item.order-item").deleteMany({
            where: { id: { $in: ownedIds } },
          });
        }
      } catch (err) {
        strapi.log.error("delete order-items failed", {
          deletedIds: normalizedDeletedIds,
          orderId: normalizedOrderId,
          err: err?.message,
          stack: err?.stack,
        });
        // Surface the error for debugging during dev
        ctx.status = 500;
        return ctx.send({
          message: "Delete failed",
          code: "ORDER_ITEM_DELETE",
          details: err?.message,
        });
      }
    }
    ...
    const orderItemService = strapi.service("api::order-item.order-item");
    return ctx.send(
      await orderItemService.buildItemsByOrderResponse(order_code)
    );
  },
};
